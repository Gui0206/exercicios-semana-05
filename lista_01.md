...
function somaArray(numeros) {
    // Inicializa a variável soma antes do loop para acumular os valores corretamente.
    let soma = 0;

    // O 'i' precisa ser declarado com 'let' para não causar erro de escopo.
    for (let i = 0; i < numeros.length; i++) { // Corrigido 'size' para 'length'.
        soma += 2 * numeros[i]; // Multiplica cada número por 2 e adiciona à soma.
    }

    return soma; // Retorna o valor acumulado corretamente.
}
...
```javascript
// Classe Produto representa um produto genérico com nome e preço
class Produto {
...
// Classe Livro herda de Produto e redefine o método calcularDesconto
class Livro extends Produto {
  constructor(nome, preco, autor) {
    super(nome, preco); // Chama o construtor da classe pai (Produto)
    this.autor = autor; // Atributo adicional exclusivo para livros
  }

  // Sobrescrevendo (override) o método calcularDesconto para aplicar 20% de desconto
  calcularDesconto() {
